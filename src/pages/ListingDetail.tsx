...
export default function ListingDetail() {
  ...
  const isSellerMode = mode === "view" || mode === "edit";
  ...
  const inputClass = "w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring";
    ...
    <div className="min-h-screen pt-16">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
          <Link to={isSellerMode ? "/seller-listings" : "/"} className="flex items-center gap-1 hover:text-foreground">
            <ArrowLeft className="h-4 w-4" /> {isSellerMode ? "Your Listings" : "Browse"}
          </Link>
          <span>/</span>
          <span className="text-foreground">{listing.title}</span>
          {isEditMode && <span className="ml-1 rounded-md bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary">Editing</span>}
        </div>

        <div className={`grid gap-8 ${isSellerMode ? "" : "lg:grid-cols-3"}`}>
          {/* Left: Images + Details */}
          <div className={isSellerMode ? "" : "lg:col-span-2"}>
              ...
              {isEditMode && editForm ? (
                <div className="space-y-6">
                  <div className="rounded-xl border border-border bg-card p-6 space-y-4">
                    <h2 className="font-display text-lg font-semibold text-foreground">Edit Listing Details</h2>
                    <div>
                    ...
                    <div>
                      ...
                      <textarea value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} rows={4} className={inputClass} />
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      <div>
                        <label className="mb-1.5 block text-sm font-medium text-foreground">Price</label>
                        <input type="number" value={editForm.price} onChange={(e) => setEditForm({ ...editForm, price: Number(e.target.value) })} className={inputClass} />
                      </div>
                      <div>
                        <label className="mb-1.5 block text-sm font-medium text-foreground">Bedrooms</label>
                        <input type="number" value={editForm.bedrooms} onChange={(e) => setEditForm({ ...editForm, bedrooms: Number(e.target.value) })} className={inputClass} />
                      </div>
                      <div>
                        <label className="mb-1.5 block text-sm font-medium text-foreground">Bathrooms</label>
                        <input type="number" value={editForm.bathrooms} onChange={(e) => setEditForm({ ...editForm, bathrooms: Number(e.target.value) })} className={inputClass} />
                      </div>
                      <div>
                        <label className="mb-1.5 block text-sm font-medium text-foreground">Sq Ft</label>
                        <input type="number" value={editForm.sqft} onChange={(e) => setEditForm({ ...editForm, sqft: Number(e.target.value) })} className={inputClass} />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      ...
                      <div>
                        ...
                        <input value={editForm.state} onChange={(e) => setEditForm({ ...editForm, state: e.target.value })} className={inputClass} />
                      </div>
                    </div>
          ...
          {/* Right sidebar: Apply — only for buyers */}
          {!isSellerMode && (
                ...
                <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                  <div className="space-y-3">
                    <div>
                      ...
                      <input value={applyForm.lastName} onChange={(e) => setApplyForm(f => ({ ...f, lastName: e.target.value }))} className={inputClass} placeholder="Doe" />
                    </div>
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-foreground">First Name</label>
                      <input value={applyForm.firstName} onChange={(e) => setApplyForm(f => ({ ...f, firstName: e.target.value }))} className={inputClass} placeholder="Jane" />
                    </div>
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-foreground">Date of Birth</label>
                      <input value={applyForm.dob} onChange={(e) => setApplyForm(f => ({ ...f, dob: e.target.value }))} className={inputClass} placeholder="MM/DD/YY" />
                    </div>
                    <div>
                      <label className="mb-1.5 block text-sm font-medium text-foreground">Email</label>
                      <input type="email" value={applyForm.email} onChange={(e) => setApplyForm(f => ({ ...f, email: e.target.value }))} className={inputClass} placeholder="you@example.com" />
                    </div>
                  </div>
